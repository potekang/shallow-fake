...
def create_inception_graph(pth):
    ...
    with tfv1.io.gfile.GFile( pth, 'rb') as f:
        #graph_def = tfv1.compat.v1.GraphDef()
        graph_def = tfv1.GraphDef()
        graph_def.ParseFromString( f.read())
        _ = tfv1.import_graph_def( graph_def, name='FID_Inception_Net')
#-------------------------------------------------------------------------------
            ...
            if shape._dims != []:
              ...
              for j, s in enumerate(shape):
                if s == 1 and j == 0:
                  ...
                  new_shape.append(s)
              o.__dict__['_shape_val'] = tfv1.TensorShape(new_shape)
    return pool3
#-------------------------------------------------------------------------------
